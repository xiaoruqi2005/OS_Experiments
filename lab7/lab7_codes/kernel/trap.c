...
void usertrap(void)
{
    ...
    else if((cause & 0x8000000000000000L) && devintr() != 0) {
        // --- 实验7 新增：处理设备中断 ---
        // 如果是设备中断（时钟、磁盘等），devintr() 会返回 1
        // 这里什么都不用做，devintr 已经处理了
        // 如果是时钟中断，可以让出 CPU
    ...
    if(killed(p))
        exit_proc(-1);
    if(cause == (0x8000000000000000L | IRQ_S_TIMER))
        yield();
